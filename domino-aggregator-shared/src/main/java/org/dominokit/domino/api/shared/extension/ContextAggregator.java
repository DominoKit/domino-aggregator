...
public class ContextAggregator {
  ...
  private ContextAggregator(Set<ContextWait> contexts, ReadyHandler handler) {
    this.handler = handler;
    contexts.forEach(this::setupContext);
  }

  /**
   * prepare a context and add it to the set of elements that needs to be completed before the
   * aggregator onReady method is called.
   *
   * @param contextWait
   */
  ...
  /**
   * Will reset all context wait elements. all context wait elements will be considered no completed
   * after this.
   */
  public void reset() {
  ...
  private static class ContextAggregatorBuilder implements CanWaitForContext {
    ...
    /**
     * a builder method to set the ContextAggregator onRead method and build the ContextAggregator
     * instance. the onReady handler will be called once all ContextWait elements are completed
     *
     * @param handler
  ...
  public static class ContextWait<T> {
    ...
    /**
     * get the value that was used to complete this context if not completed this will return null
     *
     * @return
     */
    public T get() {
